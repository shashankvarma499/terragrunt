...
- `--allow=fs-rw` gives tools access to the real filesystem.

  Without it every call works against a copy of the tree held in memory to ensure that operations are read-only. Some operations necessarily require writing to disk (resolving a dependency downloads its source into a `.terragrunt-cache`, and a `generate` block writes files within the cache directory). `--allow=exec` requires this capability, since a spawned process reads the real disk rather than the in-memory copy.
...

{
  "mcpServers": {
    "terragrunt": {
      "command": "terragrunt",
      "args": ["mcp", "--allow=exec", "--allow=fs-rw", "--allow=http"],
      "env": {
        "TG_EXPERIMENT": "mcp-command"
      }
    }
