...
// Bounds on the copy each call takes. A tree past either is refused rather
// than half-copied, since answering from a partial tree would describe an
// estate that does not exist.
const (
	mirrorMaxFiles = 20000
	mirrorMaxBytes = 256 << 20
)
...
func Serve(
	...
	deps := &serverDeps{
		...
		allowFSRW:     granted.Has(CapabilityFSRW),
...
func serverInstructions(d *serverDeps) string {
	...
	for _, c := range []struct {
		...
		{
			granted: d.allowFSRW,
			on: "The server was started with --allow=fs-rw, so what a tool writes lands in the " +
				"real tree. ",
			off: "Each tool call works against a copy of the tree held in memory. Anything a call " +
				"writes, such as a downloaded module or a file a generate block produces, goes to the copy " +
				"and is discarded when the call ends, so do not tell the user a file was written unless the " +
				"server was started with --allow=fs-rw. The copy is taken fresh per call, so an edit the " +
				"user makes between two calls is visible to the second. ",
		},
		{
			granted: d.allowExec,
