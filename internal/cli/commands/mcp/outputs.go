...
func outputsFetchViaRun(
	...
	cv, err := d.callVenv(rootVenv, env, buf)
	if err != nil {
		return getOutputsOutput{}, err
	}
	...
	raw, err := outputsExtractJSON(buf.Bytes())
	if err != nil {
...
// outputsExtractJSON isolates the first JSON object in data. Log lines can
// precede it on the captured stream, and warnings can follow it.
func outputsExtractJSON(data []byte) ([]byte, error) {
	start := bytes.IndexByte(data, '{')
	if start < 0 {
		return nil, errors.New("no JSON object found in the captured output")
	}
