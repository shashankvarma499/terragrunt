...
// separated list needs, since --allow=exec, fs-rw reaches us with the space
// attached to the second value.
func TestParseCapabilitiesIgnoresSurroundingSpace(t *testing.T) {
	t.Parallel()

	granted, err := tgmcp.ParseCapabilities([]string{" exec", "fs-rw "})
	require.NoError(t, err)

	assert.True(t, granted.Has(tgmcp.CapabilityExec))
	assert.True(t, granted.Has(tgmcp.CapabilityFSRW))
}
...
func TestParseCapabilitiesGrantsWhatWasNamed(t *testing.T) {
	...
	assert.False(t, granted.Has(tgmcp.CapabilitySops))
}
...
func TestParseCapabilitiesGrantsNothingByDefault(t *testing.T) {
	...
	for _, c := range []tgmcp.Capability{
		tgmcp.CapabilityExec, tgmcp.CapabilityHTTP, tgmcp.CapabilitySops, tgmcp.CapabilityFSRW,
	} {
