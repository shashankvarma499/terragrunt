...
// newRealFSSession is [newTestSessionWithOptions] for a test whose subject
// needs the real filesystem, which is what --allow=fs-rw grants.
func newRealFSSession(t *testing.T, dir string, configure func(*tgmcp.Options)) *mcp.ClientSession {
	t.Helper()

	return newTestSessionWith(t, dir, venvtest.NewWithOSFS(), func(o *tgmcp.Options) {
		o.Allow = append(o.Allow, "fs-rw")
		configure(o)
	}, nil)
}
...
func newRealExecSession(
	...
	return newTestSessionWith(t, dir, venvtest.NewOSWithEmptyEnv(), func(o *tgmcp.Options) {
		o.Allow = append(o.Allow, "fs-rw")
		configure(o)
	}, nil)
}
...
func TestMutatingToolsAreAbsentUntilGranted(t *testing.T) {
	...
	granted := newRealFSSession(t, newTestTree(t), func(o *tgmcp.Options) {
		o.Allow = []string{"exec", "fs-rw"}
		o.AllowApply = true
	})
	assert.Equal(
...
func TestToolAnnotationsFollowTheGrantedCapabilities(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:      "exec granted",
			configure: func(o *tgmcp.Options) { o.Allow = []string{"exec", "fs-rw"} },
			openWorld: true,
		},
	} {
...
func TestAllowExecDoesNotRunProgramsFromConfiguration(t *testing.T) {
	...
	session := newRealExecSession(
		t,
		dir,
		func(o *tgmcp.Options) { o.Allow = []string{"exec", "fs-rw"} },
	)
...
func TestOperatorTFPathIsHonoured(t *testing.T) {
	...
	session := newRealFSSession(t, dir, func(o *tgmcp.Options) {
		o.Allow = []string{"exec", "fs-rw"}
		o.TFPath = tfPath
...
// TestToolCallsLeaveTheWorkingCopyAlone pins what --allow=fs-rw denies. A
// dependency fetch downloads its source into a .terragrunt-cache, so without
// the flag a call that only reads would otherwise leave that behind in the
// tree the agent was pointed at.
func TestToolCallsLeaveTheWorkingCopyAlone(t *testing.T) {
	t.Parallel()

	dir := newTestTree(t)
	fsys := vfs.NewOSFS()

	before, err := vfs.ReadDir(fsys, filepath.Join(dir, "a"))
	require.NoError(t, err)
	...
	after, err := vfs.ReadDir(fsys, filepath.Join(dir, "a"))
	require.NoError(t, err)

	assert.Len(t, after, len(before), "a read-only call must not add anything to the tree")
}

// TestEachCallSeesTheTreeAsItIsNow pins that the copy is taken per call. An
// agent edits a unit and asks again, so a copy taken once when the client
// connected would keep answering about a file that no longer exists.
