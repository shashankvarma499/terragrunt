...
// grant that.
var execAllowedBinaries = []string{"tofu", "terraform", "opentofu", "git"}
...
func (r *execRecorder) notes() []string {
	...
	notes := make([]string, 0, len(r.order)+1)

	for _, note := range r.order {
		if n := r.counts[note]; n > 1 {
			notes = append(notes, fmt.Sprintf("%s (%d invocations)", note, n))
...
func (r *execRecorder) record(note string) {
	...
	if _, seen := r.counts[note]; !seen && len(r.order) >= execNotesMaxDistinct {
		r.suppressed++
...
func (d *serverDeps) callVenv(
	...
	if !d.allowFSRW {
		// A fresh copy per call rather than one per session, so a tool sees
		// the tree as it is now. An agent that edits a unit and asks again
		// would otherwise keep being answered from the tree as it was when
		// the client connected.
		mem, err := vfs.MirrorToMem(rootVenv.FS, d.launchDir, vfs.MirrorLimits{
			MaxFiles: mirrorMaxFiles,
			MaxBytes: mirrorMaxBytes,
		})
		if err != nil {
			return nil, fmt.Errorf("copying %s to work against: %w", d.launchDir, err)
		}
	...
	if !d.allowExec {
		return v.WithHandler(denyAllHandler(d.rec)), nil
	}
