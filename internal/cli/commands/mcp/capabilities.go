...
const (
	...
	// CapabilityFSRW lets the server read and write the real filesystem
	// rather than a per-call copy of it.
	CapabilityFSRW Capability = "fs-rw"
)

// allCapabilities is every capability --allow accepts, in the order the error
// for an unknown one lists them.
var allCapabilities = []Capability{CapabilityExec, CapabilityHTTP, CapabilitySops, CapabilityFSRW}
